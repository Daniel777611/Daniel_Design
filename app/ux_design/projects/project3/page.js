...
export default function Project3() {
    ...
    const headerVideoFullscreenRef = useRef(false);
    ...
    const fullscreenSourceRef = useRef(null); // 'video' | 'image' | 'headerVideo' | null
    ...
    useEffect(() => {
        ...
        const handleFullscreenChange = () => {
            ...
            if (isNowFullscreen && fullscreenSourceRef.current === 'video') {
                ...
                // 退出全屏：视频全屏时滚回视频区；图片全屏时滚动到当前图片位置；头部视频退出时静音
                ...
                } else if (fullscreenSourceRef.current === 'headerVideo') {
                    // 头部视频退出全屏时，恢复静音
                    if (headerVideoRef.current) {
                        headerVideoRef.current.muted = true;
                    }
                    headerVideoFullscreenRef.current = false;
                ...
                <button
                 ...
                 onClick={() => {
                    ...
                    if (video) {
                        video.currentTime = 0;
                        video.muted = false;
                        headerVideoFullscreenRef.current = true;
                        fullscreenSourceRef.current = 'headerVideo';
